...
boru-driven command-line client **and** interactive REPL for the StoicismQuote
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/stoicism-quote-cli)
make build

# 2. See usage (words, entities, env vars)
./stoicism-quote-cli --help

# 3. Provide credentials once, via the environment
export STOICISM_QUOTE_APIKEY=sk_live_xxx

# 4. Each command line is ONE boru expression, run against the API:
./stoicism-quote-cli load 1 stoic_quote            # {id:1} shorthand
./stoicism-quote-cli load '{id:1}' stoic_quote       # explicit match map

# 5. Override the API base URL for a single call
STOICISM_QUOTE_BASE=https://api.example.com ./stoicism-quote-cli load 1 stoic_quote

# 6. No arguments -> interactive REPL
./stoicism-quote-cli
stoicism-quote> load 1 stoic_quote
stoicism-quote> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/stoicism-quote-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/stoicism-quote-cli load 1 stoic_quote
   ```
...
```sh
./stoicism-quote-cli load 1 stoic_quote          # scalar shorthand for {id:1}
./stoicism-quote-cli load '{id:1}' stoic_quote     # explicit match map
```
...
./stoicism-quote-cli load 1 stoic_quote
```
...
Run with no arguments to open a REPL (prompt `stoicism-quote>`). Each line is
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `stoic_quote`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/stoicism-quote-cli`. |
...
(`./stoicism-quote-cli <expr>`) and interactively (the REPL), and expressions compose the
