// Command StoicismQuote-cli is a CLI + REPL for the StoicismQuote SDK,
...
const prompt = "stoicism-quote"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "stoic_quote"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      STOICISM_QUOTE_APIKEY (api key), STOICISM_QUOTE_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: STOICISM_QUOTE_APIKEY carries the API key and
	// STOICISM_QUOTE_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("STOICISM_QUOTE_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("STOICISM_QUOTE_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewStoicismQuoteSDK(opts)
