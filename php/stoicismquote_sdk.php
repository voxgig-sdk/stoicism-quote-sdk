...
class StoicismQuoteSDK
{
    ...
    public function __construct(array $options = [])
    {
        ...
        $feature_opts = StoicismQuoteHelpers::to_map(Struct::getprop($this->options, "feature"));
        if ($feature_opts) {
            $featureorder = Struct::getpath($this->options, "__derived__.featureorder");
            if (is_array($featureorder)) {
                foreach ($featureorder as $fname) {
                    $fopts = StoicismQuoteHelpers::to_map($feature_opts[$fname] ?? null);
                    if ($fopts && isset($fopts["active"]) && $fopts["active"] === true) {
                        ...
                        if (!StoicismQuoteFeatures::has_feature($fname)) {
                        ...
                        ($utility->feature_add)($this->_rootctx, StoicismQuoteFeatures::make_feature($fname));
                    }
                }
            }
        }
    ...
    {
        return StoicismQuoteUtility::copy($this->_utility);
    }
    ...
    public function prepare(array $fetchargs = []): mixed
    {
        ...
        $ctrl = StoicismQuoteHelpers::to_map(Struct::getprop($fetchargs, "ctrl")) ?? [];
        ...
        $params = StoicismQuoteHelpers::to_map(Struct::getprop($fetchargs, "params")) ?? [];
        $query = StoicismQuoteHelpers::to_map(Struct::getprop($fetchargs, "query")) ?? [];
    ...
    private function op_denied(string $op): array
    {
        ...
        return [
            "ok" => false,
            "err" => new StoicismQuoteError($op . "_allow",
                "StoicismQuoteSDK: " . $op . ": operation not allowed by" .
    ...
    private function raw_request(array $fetchargs = []): mixed
    {
        ...
        if (is_array($fetched)) {
            $status = StoicismQuoteHelpers::to_int(Struct::getprop($fetched, "status"));
    ...
    public function graphql(string $query, ?array $variables = null, ?array $ctrl = null): mixed
    {
        ...
        if (is_array($errors) && 0 < count($errors)) {
            ...
            $res["ok"] = false;
            $res["err"] = new StoicismQuoteError("graphql_error",
                "StoicismQuoteSDK: graphql: " . $msg);
            $res["graphql"] = $errors;
        }
    ...
    public function StoicQuote($data = null)
    {
        ...
        if ($data === null) {
            if ($this->_stoic_quote === null) {
                $this->_stoic_quote = new StoicQuoteEntity($this, null);
            }
            return $this->_stoic_quote;
        }
        return new StoicQuoteEntity($this, $data);
    }
    ...
    public static function test(?array $testopts = null, ?array $sdkopts = null): self
    {
        ...
        $sdk = new StoicismQuoteSDK($sdkopts);
        $sdk->mode = "test";
        return $sdk;
    }
}
