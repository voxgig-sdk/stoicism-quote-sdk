...
// StoicismQuote SDK pipeline test
//
...
{
    private static function utility(): StoicismQuoteUtility
    {
        return new StoicismQuoteUtility();
    }
    ...
    private static function ctx(array $over = []): StoicismQuoteContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new StoicismQuoteContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new StoicismQuoteOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof StoicismQuoteError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new StoicismQuoteOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = StoicismQuoteMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new StoicismQuoteOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = StoicismQuoteMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new StoicismQuoteOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = StoicismQuoteMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new StoicismQuoteSpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = StoicismQuoteMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        $ctx = self::ctx([
            'spec' => null,
            'response' => new StoicismQuoteResponse([]),
            'result' => new StoicismQuoteResult([]),
        ]);
        [, $err] = StoicismQuoteMakeResponse::call($ctx);
        $this->assertSame('response_no_spec', self::code($err));

        $ctx = self::ctx([
            'spec' => new StoicismQuoteSpec([]),
            'response' => null,
            'result' => new StoicismQuoteResult([]),
        ]);
        [, $err] = StoicismQuoteMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new StoicismQuoteSpec(['step' => 's']),
            'response' => new StoicismQuoteResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new StoicismQuoteResult([]),
        ]);
        [, $err] = StoicismQuoteMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new StoicismQuoteSpec(['step' => 's']),
            'response' => new StoicismQuoteResponse(self::resp(200, ['v' => 1])),
            'result' => new StoicismQuoteResult([]),
        ]);
        [, $err] = StoicismQuoteMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new StoicismQuoteSpec(['step' => 's']),
            'response' => new StoicismQuoteResponse(self::resp(200, ['v' => 2])),
            'result' => new StoicismQuoteResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        StoicismQuoteMakeResponse::call($ctx);
    ...
    {
        $preset = new StoicismQuoteResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new StoicismQuoteSpec([]),
            'response' => new StoicismQuoteResponse([]),
            'result' => new StoicismQuoteResult([]),
        ...
        [$got, $err] = StoicismQuoteMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new StoicismQuoteResult([])]);
        [, $err] = StoicismQuoteMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new StoicismQuoteSpec([]), 'result' => null]);
        [, $err] = StoicismQuoteMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new StoicismQuoteSpec(['step' => 's']),
            'result' => new StoicismQuoteResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new StoicismQuoteOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = StoicismQuoteMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new StoicismQuoteSpec(['step' => 's']),
            'result' => new StoicismQuoteResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new StoicismQuoteOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = StoicismQuoteMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new StoicismQuoteResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new StoicismQuoteSpec([]),
            'result' => new StoicismQuoteResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = StoicismQuoteMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_request_guards_a_missing_spec(): void
    {
        ...
        [, $err] = StoicismQuoteMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $boom = new StoicismQuoteError('boom', 'boom');
        $utility->fetcher = function (StoicismQuoteContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new StoicismQuoteSpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = StoicismQuoteMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (StoicismQuoteContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = StoicismQuoteMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(StoicismQuoteResponse::class, $response);
    ...
    public function test_make_request_records_the_fetchdef_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->explain = ['on' => true];
        StoicismQuoteMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (StoicismQuoteContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new StoicismQuoteSpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = StoicismQuoteMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new StoicismQuoteResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new StoicismQuoteSpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = StoicismQuoteMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = StoicismQuoteMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = StoicismQuoteMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new StoicismQuoteResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, StoicismQuoteDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new StoicismQuoteResult(['ok' => false])]);
        $this->expectException(StoicismQuoteError::class);
        StoicismQuoteDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new StoicismQuoteResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', StoicismQuoteMakeError::call($ctx, null));
    }
    ...
    public function test_make_error_records_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->throw_err = false;
        $ctx->ctrl->explain = ['on' => true];
        StoicismQuoteMakeError::call($ctx, null);
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new StoicismQuoteBaseFeature();
        $b = new StoicismQuoteBaseFeature();
        StoicismQuoteFeatureAdd::call($ctx, $a);
        StoicismQuoteFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): StoicismQuoteBaseFeature
    {
        $f = new StoicismQuoteBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    public function test_feature_add_ordering_before_after_replace(): void
    {
        ...
        StoicismQuoteFeatureAdd::call($ctx, $miss);
    ...
    private static function auth_ctx(array $options, ?array $headers): StoicismQuoteContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new StoicismQuoteSpec(['headers' => $headers]),
        ]);
    }
    ...
    public function test_prepare_auth_guards_a_missing_spec(): void
    {
        ...
        [, $err] = StoicismQuotePrepareAuth::call($ctx);
    ...
    public function test_prepare_auth_a_raw_apikey_goes_in_as_is(): void
    {
        ...
        StoicismQuotePrepareAuth::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'response' => new StoicismQuoteResponse(['headers' => null]),
            'result' => new StoicismQuoteResult([]),
        ]);
        StoicismQuoteResultHeaders::call($ctx);
    ...
    public function test_result_body_skips_parsing_when_the_body_is_absent(): void
    {
        ...
        StoicismQuoteResultBody::call($ctx);
