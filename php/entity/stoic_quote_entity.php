...
// StoicismQuote SDK StoicQuote entity
...
class StoicQuoteEntity
{
    ...
    {
        $opts = $this->_entopts;
        return new StoicQuoteEntity($this->_client, $opts);
    }

    /**
     * @param StoicQuote|array $args StoicQuote data (assoc-array) to store.
     */
    public function data_set($args): void
    {
        if ($args) {
            $this->_data = StoicismQuoteHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetData");
        }
    }

    /**
     * @return StoicQuote|array The current StoicQuote data as an assoc-array.
     */
    ...
    /**
     * @param array $args Match filter (any subset of StoicQuote fields).
     */
    public function match_set($args): void
    {
        if ($args) {
            $this->_match = StoicismQuoteHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetMatch");
        }
    }

    /**
     * @return array The current match filter (any subset of StoicQuote fields).
     */
    ...
    /**
     * Load a single StoicQuote.
     *
     * @param StoicQuoteLoadMatch|array|null $reqmatch Match criteria (id/query
     *   fields) as an assoc-array; a typed StoicQuoteLoadMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return StoicQuote|array The loaded StoicQuote as an assoc-array at the
     *   SDK boundary; throws StoicismQuoteError on failure (item-5 convention).
     */
        ...
        return $this->_run_op($ctx, function () use ($ctx) {
            if ($ctx->result) {
                ...
                if ($ctx->result->resdata) {
                    $this->_data = StoicismQuoteHelpers::to_map(Struct::clone($ctx->result->resdata)) ?? [];
                }
            }
        });
    }
