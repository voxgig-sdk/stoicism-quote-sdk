...
describe('StoicQuoteDirect', async () => {
  ...
  test('direct-exists', async () => {
    const sdk = new StoicismQuoteSDK({
  ...
  test('direct-load-stoic_quote', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-stoic_quote', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['STOICISM_QUOTE_TEST_STOIC_QUOTE_ENTID']
