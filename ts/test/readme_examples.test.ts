...
//     (\`new StoicismQuoteSDK(...)\` / \`StoicismQuoteSDK.test(...)\` become
//     \`StoicismQuoteSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const SDK_NAME = 'StoicismQuoteSDK'
...
const TEST_SEED = {"entity":{"stoic_quote":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new StoicismQuoteSDK(...)\` and any
// \`StoicismQuoteSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+StoicismQuoteSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/StoicismQuoteSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
