...
function envOverride(m: Record<string, any>) {
  if (
    'TRUE' === process.env.STOICISMQUOTE_TEST_LIVE ||
    'TRUE' === process.env.STOICISMQUOTE_TEST_OVERRIDE
  ) {
  ...
  m.STOICISMQUOTE_TEST_EXPLAIN = process.env.STOICISMQUOTE_TEST_EXPLAIN || m.STOICISMQUOTE_TEST_EXPLAIN
