...
[MCP](https://modelcontextprotocol.io) server exposing the StoicismQuote SDK as
two agent tools — `stoicism-quote_list` and `stoicism-quote_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/stoicism-quote-mcp)
make build

# 2. Provide credentials via the environment
export STOICISM_QUOTE_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user stoicism-quote \
  -- /absolute/path/to/stoicism-quote-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./stoicism-quote-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// stoicism-quote_list: first page of records
{ "entity": "stoic_quote" }
{ "entity": "stoic_quote", "query": { } }

// stoicism-quote_load: one record by id
{ "entity": "stoic_quote", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/stoicism-quote-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user stoicism-quote \
     -- "$PWD"/dist/*/stoicism-quote-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `stoicism-quote_list` and `stoicism-quote_load` tools now appear
   in new sessions. Ask the agent to *"list stoic_quote using stoicism-quote"*
   and it calls `stoicism-quote_list` with `{"entity":"stoic_quote"}`.
...
```sh
export STOICISM_QUOTE_APIKEY=sk_live_xxx            # API key
export STOICISM_QUOTE_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `stoicism-quote_list` tool
...
### Call the `stoicism-quote_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `stoicism-quote_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `stoicism-quote_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `STOICISM_QUOTE_APIKEY` | API key sent with every request. |
| `STOICISM_QUOTE_BASE` | Optional override of the API base URL. |
...
```sh
./stoicism-quote-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"stoicism-quote_load","arguments":{"entity":"stoic_quote","query":{"id":1}}}}'
```
