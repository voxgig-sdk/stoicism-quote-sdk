...
class StoicQuoteEntityTest < Minitest::Test
  def test_create_instance
    testsdk = StoicismQuoteSDK.test(nil, nil)
    ent = testsdk.StoicQuote(nil)
    assert !ent.nil?
  end

  def test_basic_flow
    setup = stoic_quote_basic_setup(nil)
    # Per-op sdk-test-control.json skip.
    _live = setup[:live] || false
    ["load"].each do |_op|
      _should_skip, _reason = Runner.is_control_skipped("entityOp", "stoic_quote." + _op, _live ? "live" : "unit")
      if _should_skip
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set STOICISMQUOTE_TEST_STOIC_QUOTE_ENTID JSON to run live"
      return
    end
    client = setup[:client]
    ...
    stoic_quote_ref01_data = nil
    if stoic_quote_ref01_data_raw.length > 0
...
def stoic_quote_basic_setup(extra)
  Runner.load_env_local

  entity_data_file = File.join(__dir__, "..", "..", ".sdk", "test", "entity", "stoic_quote", "StoicQuoteTestData.json")
  ...
  entid_env_raw = ENV["STOICISMQUOTE_TEST_STOIC_QUOTE_ENTID"]
  ...
  if env["STOICISMQUOTE_TEST_LIVE"] == "TRUE"
    ...
    client = StoicismQuoteSDK.new(Helpers.to_map(merged_opts))
  end
