...
class StoicQuoteEntity
  ...
  def make
    opts = @_entopts.dup
    StoicQuoteEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = StoicismQuoteHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [StoicQuote, Hash] the current StoicQuote data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = StoicismQuoteHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of StoicQuote fields)
  def match_get
  ...
  #
  # @param reqmatch [StoicQuoteLoadMatch, Hash, nil] match criteria (id/query fields);
  #   optional — an entity with no id-like key loads with no match (nil is treated
  #   as an empty match, so client.StoicQuote.load works with no args).
  # @param ctrl [Object, nil] optional per-call control
  # @return [StoicQuote, Hash] the loaded StoicQuote; raises StoicismQuoteError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = StoicismQuoteHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
