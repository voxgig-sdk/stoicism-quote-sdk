...
require_relative 'StoicismQuote_types'
...
class StoicismQuoteSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = StoicismQuoteHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = StoicismQuoteHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, StoicismQuoteFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    StoicismQuoteUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = StoicismQuoteHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = StoicismQuoteHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = StoicismQuoteHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def raw_request(fetchargs = {})
    ...
    if fetched.is_a?(Hash)
      status = StoicismQuoteHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = StoicismQuoteError.new(
        "graphql_error", "StoicismQuoteSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
  ...
  def self.test(testopts = nil, sdkopts = nil)
    ...
    sdk = StoicismQuoteSDK.new(sdkopts)
    sdk.mode = "test"
    sdk
  end
end
