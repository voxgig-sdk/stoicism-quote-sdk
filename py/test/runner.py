...
class StoicismQuoteTestRunner:
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            StoicismQuoteTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = StoicismQuoteTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = StoicismQuoteTestRunner.getenv("STOICISM_QUOTE_TEST_LIVE")
        override = StoicismQuoteTestRunner.getenv("STOICISM_QUOTE_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = StoicismQuoteTestRunner.getenv(key)
        ...
        explain = StoicismQuoteTestRunner.getenv("STOICISM_QUOTE_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["STOICISM_QUOTE_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if StoicismQuoteTestRunner._test_control is not None:
            return StoicismQuoteTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                StoicismQuoteTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            StoicismQuoteTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = StoicismQuoteTestRunner.load_test_control()
...
def load_env_local():
    StoicismQuoteTestRunner.load_env_local()


def env_override(m):
    return StoicismQuoteTestRunner.env_override(m)


def entity_data(v):
    return StoicismQuoteTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return StoicismQuoteTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return StoicismQuoteTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return StoicismQuoteTestRunner.load_test_control()


def live_client_options():
    return StoicismQuoteTestRunner.live_client_options()


def live_delay_ms():
    return StoicismQuoteTestRunner.live_delay_ms()
