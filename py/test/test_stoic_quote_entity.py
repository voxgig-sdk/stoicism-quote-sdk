    ...
    def test_should_run_basic_flow(self):
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set STOICISM_QUOTE_TEST_STOIC_QUOTE_ENTID JSON to run live")
        client = setup["client"]
        ...
        stoic_quote_ref01_data_dt0_loaded = stoic_quote_ref01_ent.load(stoic_quote_ref01_match_dt0, None)
...
def _stoic_quote_basic_setup(extra):
    ...
    idmap_resolved = helpers.to_map(
        env.get("STOICISM_QUOTE_TEST_STOIC_QUOTE_ENTID"))
    if idmap_resolved is None:
        idmap_resolved = helpers.to_map(idmap)

    if env.get("STOICISM_QUOTE_TEST_LIVE") == "TRUE":
    ...
    _live = env.get("STOICISM_QUOTE_TEST_LIVE") == "TRUE"
    return {
        "client": client,
        "data": entity_data,
        "idmap": idmap_resolved,
        "env": env,
        "explain": env.get("STOICISM_QUOTE_TEST_EXPLAIN") == "TRUE",
        "live": _live,
