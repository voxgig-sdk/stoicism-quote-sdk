...
local StoicQuoteEntity = {}
StoicQuoteEntity.__index = StoicQuoteEntity


function StoicQuoteEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, StoicQuoteEntity)
  self._name = "stoic_quote"
  self._client = client
...
function StoicQuoteEntity:get_name()
  return self._name
end


function StoicQuoteEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return StoicQuoteEntity.new(self._client, opts)
end
...
function StoicQuoteEntity:mark_deleted()
  self._deleted = true
end


function StoicQuoteEntity:deleted()
  return true == self._deleted
end


function StoicQuoteEntity:data_set(args)
  if args ~= nil then
...
function StoicQuoteEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function StoicQuoteEntity:match_set(args)
  if args ~= nil then
...
function StoicQuoteEntity:match_get()
...
function StoicQuoteEntity:stream(action, args, callopts)
...
function StoicQuoteEntity:_run_op(ctx, post_done)
  local utility = self._utility
