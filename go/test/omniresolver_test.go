...
import (
	...
	omni "github.com/voxgig-sdk/stoicism-quote-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.StoicismQuoteError); is {
				out := map[string]any{
					"name":    "StoicismQuoteError",
...
func omniCtx(arg any, client *sdk.StoicismQuoteSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.StoicismQuoteSDK); is {
			client = live
