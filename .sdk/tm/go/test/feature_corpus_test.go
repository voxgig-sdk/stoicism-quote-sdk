...
//
// NewStoicismQuoteSDK, not TestSDK: the `test` feature is transport: 'base' and
// REPLACES the transport, so a client in test mode would shadow the script.
func fcClient(kase map[string]any) *sdk.StoicismQuoteSDK {
	res, _ := kase["res"].([]any)
	opts := map[string]any{
	...
	return sdk.NewStoicismQuoteSDK(opts)
}
...
func fcCandidates(client *sdk.StoicismQuoteSDK) []fcOp {
	out := []fcOp{}
...
func fcInvoke(client *sdk.StoicismQuoteSDK, op fcOp, ctrl map[string]any) error {
	cv := reflect.ValueOf(client)
	ct := cv.Type()
	mapType := reflect.TypeOf(map[string]any{})
...
// new go here.
func fcRecord(client *sdk.StoicismQuoteSDK, name string) any {
			...
			for _, raw := range cases {
				...
				for _, rawstep := range steps {
					...
					if code, ok := wanterr.(string); ok {
						...
						// the word.
						got := ""
						var sdkerr *sdk.StoicismQuoteError
