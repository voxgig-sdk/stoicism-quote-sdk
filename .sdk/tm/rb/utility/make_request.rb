# StoicismQuote SDK utility: make_request
...
module StoicismQuoteUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = StoicismQuoteResponse.new({})
    ...
    if fetch_err
      ...
      response = StoicismQuoteResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = StoicismQuoteResponse.new(fetched)
    else
