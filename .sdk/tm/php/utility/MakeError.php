...
class StoicismQuoteMakeError
{
    public static function call(?StoicismQuoteContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new StoicismQuoteContext([], null);
        }
        $op = $ctx->op ?? new StoicismQuoteOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new StoicismQuoteResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof StoicismQuoteError) ? $err->msg : (string)$err;
        ...
        $sdk_err = new StoicismQuoteError('', $msg, $ctx);
        ...
        if ($err instanceof StoicismQuoteError) {
