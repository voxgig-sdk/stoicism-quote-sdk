...
//    only - a typed StoicismQuoteContext there would make every ctx
...
final class StoicismQuoteOmni
{
    ...
    public static function ctx_from_map($ctxmap, $client, $utility)
    {
        ...
        $ctx = new StoicismQuoteContext($clean, null);
        ...
        if (isset($clean['spec']) && is_array($clean['spec'])) {
            $ctx->spec = new StoicismQuoteSpec($clean['spec']);
        }

        if (isset($clean['result']) && is_array($clean['result'])) {
            $ctx->result = new StoicismQuoteResult($clean['result']);
            if (isset($clean['result']['err']) && is_array($clean['result']['err'])) {
                $msg = $clean['result']['err']['message'] ?? '';
                $ctx->result->err = new StoicismQuoteError('', is_string($msg) ? $msg : '');
            }
        }

        if (isset($clean['response']) && is_array($clean['response'])) {
            $ctx->response = new StoicismQuoteResponse($clean['response']);
    ...
    public static function err_from_map($m): ?StoicismQuoteError
    {
        if (!is_array($m)) {
            return null;
        }
        ...
        $code = $m['code'] ?? '';
        return new StoicismQuoteError(is_string($code) ? $code : '', $msg);
    }
