...
// StoicismQuote SDK feature test
//
...
class FtCtrl extends StoicismQuoteControl
{
...
class FtClient
{
    ...
    private StoicismQuoteUtility $utility;

    public function __construct(StoicismQuoteUtility $utility, array $options)
    {
    ...
    public function get_utility(): StoicismQuoteUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (StoicismQuoteContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public StoicismQuoteContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (StoicismQuoteContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = StoicismQuoteConfig::make_feature($name);
    ...
    private static function buildUrl(StoicismQuoteSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof StoicismQuoteError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new StoicismQuoteResponse($fetched);
        }
    ...
    private function fail(StoicismQuoteContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (StoicismQuoteContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
