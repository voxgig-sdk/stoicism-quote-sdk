...
// StoicismQuote SDK test feature

require_once __DIR__ . '/BaseFeature.php';
require_once __DIR__ . '/../utility/Param.php';

class StoicismQuoteTestFeature extends StoicismQuoteBaseFeature
{
    ...
    public function init(StoicismQuoteContext $ctx, array $options): void
    {
        ...
        $test_fetcher = function (StoicismQuoteContext $fctx, string $_fullurl, array $_fetchdef) use ($entity): array {
            ...
            if ($op->name === 'load') {
                ...
                $id = StoicismQuoteParam::call($fctx, 'id');
    ...
    public function makeNetsim(array $net, callable $inner): callable
    {
        ...
        return function (StoicismQuoteContext $fctx, string $url, array $fetchdef) use ($net, $inner, $pick_latency, $sleep): array {
            $this->_netcalls++;
            $call = $this->_netcalls;
     ...
     */
    public function buildArgs(StoicismQuoteContext $ctx, $op, $args): array
    {
        ...
        foreach ($keys as $k) {
            ...
            if ($is_id || $in_reqd) {
                $v = StoicismQuoteParam::call($ctx, $k);
