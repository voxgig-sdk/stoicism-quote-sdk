...
// StoicismQuote SDK retry feature
...
class StoicismQuoteRetryFeature extends StoicismQuoteBaseFeature
{
    ...
    public function init(StoicismQuoteContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (StoicismQuoteContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(StoicismQuoteContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(StoicismQuoteContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
